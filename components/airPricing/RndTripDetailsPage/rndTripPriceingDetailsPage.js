...
class RndTripPriceingDetailsPage extends Component {
  ...
  prePopulateAirPricDetailsEachOne = (airPrOption) => {
    ...
    if (airPrOption) {
      ...
      //Segment Info End
      return {
        solutionInf: airSolution,
        segmentInf: segOptions,
        segments: segment,
      };
    }
  };
        ...
        airPricingInfo.map((airPriceInf, apIdx) => {
          ...
          if (airPriceInf) {
            ...
            priceInfs.push({
              ...
              eqBasePrice:
                equivalentBasePrice || approximateBasePrice || basePrice,
    ...
    if (
      ...
      if (
        ...
        let sumTotalPrice = 0,
          ...
          sumBasePrice = 0,
          currencyType = 0;
        let eAdtTotalPrice = 0,
          eAdtTotalTaxes = 0,
          eAdtEquBasePrice = 0,
          adtPassengerQty = 0,
          adtStatus = false;
        let eCnnTotalPrice = 0,
          eCnnTotalTaxes = 0,
          eCnnEquBasePrice = 0,
          cnnPassengerQty = 0,
          cnnStatus = false;
        let eInfTotalPrice = 0,
          eInfTotalTaxes = 0,
          eInfEquBasePrice = 0,
          infPassengerQty = 0,
          infStatus = false;
        ...
        itineraryPrices &&
          itineraryPrices.map((itineraryPrice, ipIdx) => {
            console.log("itineraryPrice, ", itineraryPrice);

            if (!helperIsEmpty(itineraryPrice)) {
              if (
                !helperIsEmpty(itineraryPrice.details.solutionInf.fareSummary)
              ) {
                let {
                  priceSummeries,
                  equivalentBasePrice,
                  ...
                  approximateBasePrice,
                  basePrice,
                } = itineraryPrice.details.solutionInf.fareSummary;
                ...
                console.log(
                  "itineraryPrice Price Details, priceSummeries, ",
                  priceSummeries,
                  " equivalentBasePrice, ",
                  equivalentBasePrice,
                  " totalPrice, ",
                ...
                sumTotalPrice =
                  Number(sumTotalPrice) + Number(helperGetAmount(tPrice));
                sumTaxes = Number(sumTaxes) + Number(helperGetAmount(tAmnt));
                sumBasePrice =
                  Number(sumBasePrice) + Number(helperGetAmount(bPrice));
                ...
                if (priceSummeries) {
                  priceSummeries.map((eachPrice, epIdx) => {
                    if (eachPrice) {
                      ...
                      let { key, totalPrice, taxes, eqBasePrice, passQty } =
                        eachPrice;
                      if (key === "ADT") {
                        console.log(
                          "priceSummeries, ADT !! Total Price: ",
                          eAdtTotalPrice,
                          " Amount Via Api, ",
                          totalPrice,
                          " Amount -> ",
                          helperGetAmount(totalPrice)
                        );

                        eAdtTotalPrice =
                          Number(eAdtTotalPrice) +
                          Number(helperGetAmount(totalPrice));
                        eAdtTotalTaxes =
                          Number(eAdtTotalTaxes) +
                          Number(helperGetAmount(taxes));
                        eAdtEquBasePrice =
                          Number(eAdtEquBasePrice) +
                          Number(helperGetAmount(eqBasePrice));
                      ...
                      if (key === "INF") {
                        eInfTotalPrice =
                          Number(eInfTotalPrice) +
                          Number(helperGetAmount(totalPrice));
                        eInfTotalTaxes =
                          Number(eInfTotalTaxes) +
                          Number(helperGetAmount(taxes));
                        eInfEquBasePrice =
                          Number(eInfEquBasePrice) +
                          Number(helperGetAmount(eqBasePrice));
                      ...
                      if (key === "CNN") {
                        eCnnTotalPrice =
                          Number(eCnnTotalPrice) +
                          Number(helperGetAmount(totalPrice));
                        eCnnTotalTaxes =
                          Number(eCnnTotalTaxes) +
                          Number(helperGetAmount(taxes));
                        eCnnEquBasePrice =
                          Number(eCnnEquBasePrice) +
                          Number(helperGetAmount(eqBasePrice));
        ...
        if (adtStatus) {
          eachPrices.push({
            key: "ADT",
            totalPrice: eAdtTotalPrice,
            taxes: eAdtTotalTaxes,
            eqBasePrice: eAdtEquBasePrice,
            passengerQty: adtPassengerQty,
          });
        }
        ...
        if (cnnStatus) {
          eachPrices.push({
            key: "CNN",
            totalPrice: eCnnTotalPrice,
            taxes: eCnnTotalTaxes,
            eqBasePrice: eCnnEquBasePrice,
            passengerQty: cnnPassengerQty,
          });
        }
        ...
        if (infStatus) {
          eachPrices.push({
            key: "INF",
            totalPrice: eInfTotalPrice,
            taxes: eInfTotalTaxes,
            eqBasePrice: eInfEquBasePrice,
            passengerQty: infPassengerQty,
          });
        }
        ...
        localDataStore.setPreSetRndPriceDetails({
          farePriceSummery,
          deptuerPriceDetails,
          returnPriceDetails,
        });
  ...
  render() {
    let { depPricingDetails, retPricingDetails, loadStatus } = this.state;

    if (loadStatus) {
      return (
        <LoaderSpiner
          show={loadStatus}
          loadingText="Geting Price information ..."
        />
      );
    }
                ...
                <Card>
                  <Card.Title>
                    <AirPricringItinerayTitle
                      ...
                      changePenalty={
                        depPricingDetails &&
                        depPricingDetails.solutionInf &&
                        depPricingDetails.solutionInf.changePenaltyList
                      }
                      cancelPenalty={
                        depPricingDetails &&
                        depPricingDetails.solutionInf &&
                        depPricingDetails.solutionInf.cancelPenaltyList
                      }
                      stops={
                        depPricingDetails &&
                        depPricingDetails.segmentInf &&
                        depPricingDetails.segmentInf.stops
                      }
                      // stoplocs={this.state.depPricingDetails.stopsLocs}
                      totalFlyTime={
                        depPricingDetails &&
                        depPricingDetails.segmentInf &&
                        depPricingDetails.segmentInf.totalFlyTime
                      }
                      cabinClass={
                        depPricingDetails &&
                        depPricingDetails.solutionInf &&
                        depPricingDetails.solutionInf.cabinClass
                      }
                      origin={
                        depPricingDetails &&
                        depPricingDetails.segmentInf &&
                        depPricingDetails.segmentInf.firstOrigin
                      }
                      destination={
                        depPricingDetails &&
                        depPricingDetails.segmentInf &&
                        depPricingDetails.segmentInf.lastDestination
                      }
                      departureTime={
                        depPricingDetails &&
                        depPricingDetails.segmentInf &&
                        depPricingDetails.segmentInf.fstDepTime
                      }
                    />
                  </Card.Title>
                  <Card.Body>
                    {depPricingDetails &&
                      depPricingDetails.segments &&
                      depPricingDetails.segments.map((segment, sIdx) => {
                        let deff = null;
                        if (sIdx > 0) {
                          ...
                          <React.Fragment key={`pr-ic-${sIdx}`}>
                            ...
                            <PriceItineraryCard
                              ...
                              lBrand={
                                depPricingDetails &&
                                depPricingDetails.solutionInf &&
                                depPricingDetails.solutionInf.lBrand
                              }
                              cabinClass={
                                depPricingDetails &&
                                depPricingDetails.solutionInf &&
                            ...
                            <Row className="baggage-area">
                              <PricingBaggageCard
                                baggageAllowance={
                                  depPricingDetails &&
                                  depPricingDetails.solutionInf &&
                                  depPricingDetails.solutionInf.baggageLists
                                }
                              />
                            </Row>
                          </React.Fragment>
                ...
                <Card>
                  <Card.Title>
                    <AirPricringItinerayTitle
                      ...
                      changePenalty={
                        retPricingDetails &&
                        retPricingDetails.solutionInf &&
                        retPricingDetails.solutionInf.changePenaltyList
                      }
                      cancelPenalty={
                        retPricingDetails &&
                        retPricingDetails.solutionInf &&
                        retPricingDetails.solutionInf.cancelPenaltyList
                      }
                      stops={
                        retPricingDetails &&
                        retPricingDetails.segmentInf &&
                        retPricingDetails.segmentInf.stops
                      }
                      // stoplocs={this.state.depPricingDetails.stopsLocs}
                      totalFlyTime={
                        retPricingDetails &&
                        retPricingDetails.segmentInf &&
                        retPricingDetails.segmentInf.totalFlyTime
                      }
                      cabinClass={
                        retPricingDetails &&
                        retPricingDetails.solutionInf &&
                        retPricingDetails.solutionInf.cabinClass
                      }
                      origin={
                        retPricingDetails &&
                        retPricingDetails.segmentInf &&
                        retPricingDetails.segmentInf.firstOrigin
                      }
                      destination={
                        retPricingDetails &&
                        retPricingDetails.segmentInf &&
                        retPricingDetails.segmentInf.lastDestination
                      }
                      departureTime={
                        retPricingDetails &&
                        retPricingDetails.segmentInf &&
                        retPricingDetails.segmentInf.fstDepTime
                      }
                    />
                  </Card.Title>
                  <Card.Body>
                    {retPricingDetails &&
                      retPricingDetails.segments &&
                      retPricingDetails.segments.map((segment, sIdx) => {
                        let deff = null;
                        if (sIdx > 0) {
                          deff = helperGetTotalFlyTimeBetweenTwoDate(
                            prevDate,
                        ...
                        return (
                          <React.Fragment key={`ret-pr-ic-${sIdx}`}>
                            ...
                            <PriceItineraryCard
                              ...
                              lBrand={
                                retPricingDetails.solutionInf &&
                                retPricingDetails.solutionInf.lBrand
                              }
                              cabinClass={
                                retPricingDetails.solutionInf &&
                                retPricingDetails.solutionInf.cabinClass
                            ...
                            <Row className="baggage-area">
                              <PricingBaggageCard
                                baggageAllowance={
                                  retPricingDetails &&
                                  retPricingDetails.solutionInf &&
                                  retPricingDetails.solutionInf.baggageLists
                                }
                              />
                            </Row>
                          </React.Fragment>
        ...
        <Row>
          <Col md={3}>
            <button
              type="button"
              onClick={() => {
                localDataStore.setRoundTripFarePriceSummery(
                  this.state.farePriceSummery
                );
